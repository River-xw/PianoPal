...
class TestIgnoreTiming:
    """ScoringConfig.ignore_timing drops the timing dimension entirely: a
    pitch-matched note is always `correct` (never `timing_off`), offset_ms/
    timing/timing_stability are omitted (None), and the score is pitch+
    rhythm only.
    """

    def test_a_badly_rushed_note_becomes_correct_not_timing_off(self):
        from backend.scoring.config import ScoringConfig

        reference = _build_reference()
        performance = _perfect_performance(reference)
        performance[4]["onset_sec"] -= 0.080  # would normally be flagged rush/timing_off

        result = score_performance(reference, performance, ScoringConfig(ignore_timing=True))

        assert result.summary.counts["timing_off"] == 0
        assert result.summary.counts["correct"] == len(reference["notes"])
        note = next(r for r in result.notes if r.ref_index == 4)
        assert note.status == "correct"
        assert note.offset_ms is None
        assert note.timing is None

    def test_wrong_pitch_still_classified_normally(self):
        from backend.scoring.config import ScoringConfig

        reference = _build_reference()
        performance = _perfect_performance(reference)
        performance[0]["pitch"] += 1  # a genuine wrong pitch, unrelated to timing

        result = score_performance(reference, performance, ScoringConfig(ignore_timing=True))
        assert result.summary.counts["wrong_pitch"] == 1

    def test_timing_stability_and_offset_ms_are_none(self):
        from backend.scoring.config import ScoringConfig
        ...
        assert all(r.offset_ms is None and r.timing is None for r in result.notes if r.pitch_perf is not None)

    def test_score_uses_only_pitch_and_rhythm_renormalized(self):
        from backend.scoring.config import ScoringConfig

        reference = _build_reference()
        performance = _perfect_performance(reference)
        result = score_performance(reference, performance, ScoringConfig(ignore_timing=True))
        # perfect pitch+rhythm, timing dropped -> still 100
        assert result.summary.score == pytest.approx(100.0)

    def test_default_behavior_unchanged(self):
        # ignore_timing defaults to False -- existing behavior untouched
        reference = _build_reference()
        performance = _perfect_performance(reference)
        performance[4]["onset_sec"] -= 0.080
        result = score_performance(reference, performance)
        assert result.summary.counts["timing_off"] == 1
        assert result.summary.sub_scores["timing_stability"] is not None
