...
- `ws2812_guide_song.py` + `led_keyboard.py`: lights up the WS2812 strip along the keyboard in time with the reference score ("follow mode"), with live speed/pause/restart control over HTTP; optionally records audio for the session's exact duration; supports `--no-leds` (演奏模式, timing+recording only) and `--loop-start-measure`/`--loop-end-measure` (分段循环练习, loops a measure range indefinitely).
- `posture_capture.py`: standalone subprocess that runs the BLE micro:bit + MPU6050 posture pipeline (`edge/raspi_runtime/posture.py`/`ble.py`, reused as a library) for one practice session's duration and reduces the stream of posture-classifier predictions to a single 0-100 hand-shape score. In learning mode it also sends persistent posture issues through `posture_feedback.py` to the English voice coach. Degrades gracefully (writes a `null` score, doesn't error) when no BLE rig is configured.
- `posture_feedback.py`: maps the trained model's five non-normal labels to concise English coaching cues. It requires four consecutive confident predictions and applies a ten-second global cooldown before atomically publishing a new voice event.
...
POST /api/session/start (song, mode=learn|perform, brightness, loop range...)
  ...
  -> posture_capture.py subprocess, if a BLE rig is configured (parallel, optional)
       -> posture_feedback.py -> Pi ALSA WAV cue, or browser fallback (learn mode)
