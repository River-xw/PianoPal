...
如果你已經知道使用者是對著哪個 BPM 的節拍器彈的（`target_bpm`），就不用猜測全域速度了——直接把參考樂譜換算到那個 BPM（用 `score_to_reference.to_seconds`），殘差就是絕對值，不做回歸。
...
**重要**：`scoring` 依賴同一層目錄下的 `score_to_reference`（用來呼叫 `to_seconds`），這不是 PyPI 套件，而是兄弟資料夾。執行時的**工作目錄**必須是兩者共同的上層資料夾（也就是 `學習用/`），這樣 `import score_to_reference` 才找得到路徑：

```bash
cd 學習用/          # 不是 cd scoring/
python -m scoring ...
```
...
```python
from scoring import score_performance, ScoringConfig
import json
...
```python
from scoring import midi_to_performance
...
```bash
python -m scoring reference.json performance.json -o result.json
python -m scoring reference.json performance.json -o result.json --bpm 90
```
...
| `cli.py` / `__main__.py` | `python -m scoring ...` |
