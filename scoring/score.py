...
try:
    from score_to_reference import to_seconds
except ImportError as exc:  # pragma: no cover
    raise ImportError(
        "scoring depends on the sibling score_to_reference package -- run from a "
        "working directory where both packages are importable (e.g. their shared "
        "parent directory) or add it to PYTHONPATH."
    ) from exc
...
def score_performance(
    ...
    (via score_to_reference.to_seconds) and residuals are absolute -- we trust
