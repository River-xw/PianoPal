...
```python
from score_to_reference import convert
...
```python
from score_to_reference import convert, to_seconds
...
```bash
# 轉換並印到終端機
python -m score_to_reference 我的樂譜.musicxml

# 轉換並存成檔案
python -m score_to_reference 我的樂譜.musicxml -o ref.json

# 轉換同時換算成指定練習速度（例如 90 bpm）
python -m score_to_reference 我的樂譜.mid -o ref.json --bpm 90
```
...
| `__main__.py` | CLI 入口（`python -m score_to_reference ...`） |
