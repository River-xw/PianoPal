...
給 [scoring](../scoring) 模組輸出的 `result.json` 用的網頁檢視器。純前端、不需要後端——把檔案讀進瀏覽器記憶體就直接畫圖，沒有任何資料離開你的電腦。
...
打開 `http://localhost:5173`。如果 `viewer/public/result.json` 已經存在（例如用 [`../grade.py`](../grade.py) 跑過），會自動載入顯示；不然點右上角「Load result.json」手動選一份 `python -m scoring ... -o result.json` 產生的檔案。
...
- 目前只支援本機選檔或 `viewer/public/result.json` 自動載入，沒有做後端 API 串接
