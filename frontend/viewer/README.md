...
**學習模式 vs 演奏模式**：都是 `POST /api/session/start` 帶 `mode: "learn"|"perform"`，後端依 mode 選一組 `ScoringConfig` 權重（`edge/practice_server.py`/`scripts/session_server.py` 的 `MODE_SCORE_WEIGHTS`）——學習模式旋律／動作權重高、節奏均勻度不計；演奏模式三者較均衡的嚴格評分，而且會多帶 `--no-leds` 給 `ws2812_guide_song.py`（只計時+錄音，不點燈）。**評分引擎本身完全沒有分兩套**，純粹是權重參數不同。樹莓派正式鏈路會在同一次 session 中並行啟動麥克風錄音和左手動作模型，將 `motion_score` 作為動作子分；BLE、配置或模型不可用時顯示 N/A，並從總分權重中排除，絕不使用固定佔位分。
