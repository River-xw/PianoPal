#!/usr/bin/env python3
"""Grade a student recording against a demo recording, no MIDI required."""
...
from backend.audio_to_performance.audio_reference import grade_student_against_demo  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python3 scripts/grade_against_demo_audio.py",
        description="Compare student audio to a trusted demo audio reference.",
    )
    parser.add_argument("demo_audio")
    parser.add_argument("student_audio")
    parser.add_argument("-o", "--output", required=True, help="Result JSON output path.")
...
def main() -> int:
    ...
    result, debug = grade_student_against_demo(
        args.demo_audio,
        args.student_audio,
        config,
        title=args.title or Path(args.demo_audio).stem,
    )
    ...
    print(f"demo events: {result['pipeline']['demo_audio_reference']['demo_events']}")
    print(f"student events: {result['pipeline']['demo_audio_reference']['student_events']}")
