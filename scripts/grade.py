...
Usage:
    python3 scripts/grade.py <標準樂譜> <待檢測的檔案> [--bpm 90]
...
from backend.scoring import ScoringConfig, midi_to_performance, score_performance  # noqa: E402
...
def _ensure_dev_server_running() -> bool:
    if _server_is_up():
        return True
    print("啟動前端開發伺服器（frontend/viewer/ 底下的 npm run dev）...")
    subprocess.Popen(
        ["npm", "run", "dev"],
        cwd=str(VIEWER_DIR),
    ...
    for _ in range(45):
        if _server_is_up():
            return True
        time.sleep(1)
    print("警告：等了 45 秒前端伺服器還沒起來，請自己到 frontend/viewer/ 跑 npm run dev 看看是什麼錯誤")
    return False
