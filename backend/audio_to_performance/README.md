...
## 音色不符實體樂器(`timbre_fingerprint.py`)：玩具琴 vs. basic-pitch 的真鋼琴訓練資料

實測發現：專案用的玩具琴，錄音直接跑現有 pipeline，**69個音符只對1個(分數12.16)**——遠比噪音問題(見下方真人錄音 vs 合成音檔的落差)還嚴重。直接看原始頻譜才知道為什麼：玩具琴低音鍵的**真實基頻幾乎沒有能量**——例如標記 C3(130.8Hz)的鍵，錄音裡能量最強的頻率其實在 656.7Hz(第5泛音，5倍)。小喇叭/電路發不出低頻基音，basic-pitch(在真鋼琴錄音上訓練，那裡基頻通常都在)自然會抓錯峰。這不是隨機雜訊，是這個樂器的固定物理限制。

**利用「樂器固定不變」這件事**：既然同一把琴的37個鍵，音色每次都一樣(不像真人演奏或不同鋼琴有變異)，與其套用通用的泛音假設，不如直接錄這把琴、從錄音裡學出每個鍵實際發出的頻譜「指紋」，之後拿新錄音去比對指紋，而不是靠 basic-pitch 對真鋼琴的假設。

- `extract_labeled_segments`：給一份「這把琴彈某首曲子」的錄音 + 對應參考譜，把參考譜切成單音事件(排除和弦——沒辦法歸因給單一音高)，用**音訊 onset 偵測**(不是音高偵測，攻擊瞬間偵測對音色不熟悉的樂器還算可靠)對齊到錄音裡的真實起音，每個對齊到的事件切一段 CQT
- `build_templates`：同音高的多次出現正規化後取平均，做成每個音高一個指紋向量；出現次數越多模板越穩健
- `save_templates` / `load_templates`：存成JSON(`data/toy_keyboard_fingerprints.json`)，載入時會檢查 CQT 參數(fmin/bins_per_octave/n_octaves)是否跟目前設定一致，不一致直接報錯而不是默默比錯
...
**實測(用 twinkle_take1.m4a，涵蓋37鍵裡的6個、每個1-4次)**：有指紋 vs 沒指紋，分數 13.83→20.69，`correct` 1→4，`wrong_pitch` 32→24——方向正確、有實際改善，**但離可用還很遠**。誠實的限制：只有6/37個鍵有指紋，且每個鍵樣本數極少(1-4次)，模板不夠穩健。要真正做到37鍵全覆蓋，需要更多錄音(理想上每個鍵單獨錄、或多首涵蓋不同音域的曲子)。
